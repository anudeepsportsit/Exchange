...
public enum ExecutionReportErrorCode {
    ...
    INVALID_ACCOUNT_STATE("OrderTO rejected due to the account's status (suspended, inactive, dup cards)"),
    INVALID_WALLET_STATUS("OrderTO rejected due to the account's wallet's status"),
    ...
    INVALID_ORDER("OrderTO cannot be accepted by the matcher due to the combination of actions. For example, bets being edited are not on the same market, or order includes both edits and placement"),
    ...
    NO_ACTION_REQUIRED("OrderTO hasn't been passed to matcher as system detected there will be no state change"),
